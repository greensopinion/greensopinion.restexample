...
Ideally we want to enable running tests with zero setup.  Data-intensive applications need a database, which are notoriously hard to configure and setup.  We'll use a database for our tests, but instead of connecting to an external one the database will start up and run as part of the unit test.  This gives us the following:
...
In my next article I'll post full source code and show how these techniques can be taken a step further in testing Spring REST web services.